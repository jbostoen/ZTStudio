...
Public Class ClsFrame
    ...
    Function RenderCoreImageFromHex() As ClsDirectBitmap
        ...
        ' Read cursor into LstFrameHex, used instead of repeatedly calling LstFrameHex.RemoveFirst(n)
        ' while decoding pixel rows below - RemoveFirst shifts every remaining element down (O(remaining
        ' count) per call), which across many rows/drawing instructions in a large frame made decoding
        ' effectively quadratic. Advancing a cursor keeps LstFrameHex untouched and makes this O(N).
        ...
        ' Advance the cursor past them to speed up further processing.
        ' No worries, LstFrameHex itself is a copy of the bytes And is never mutated.
        IntCursor += 10
        ...
        While IntCursor < LstFrameHex.Count And IntY < ObjFrameCoreImageBitmap.Height
            ...
            IntNumDrawingInstructions = CInt("&H" & LstFrameHex(IntCursor))
            IntCursor += 1
            ...
            For IntNumDrawingInstructions_current = 0 To (IntNumDrawingInstructions - 1)
                ...
                ' And so on.
                IntX += CInt("&H" & LstFrameHex(IntCursor))
                ' Number of pixels to color ([num of pixels to draw])
                IntNumDrawingInstructions_colors = CInt("&H" & LstFrameHex(IntCursor + 1))

                ' Advance past [offset] and [num of pixels to draw].
                IntCursor += 2
                ...
                For IntNumDrawingInstructions_colors_current = 0 To (IntNumDrawingInstructions_colors - 1)
                    ...
                    Else
                        ...
                        Dim IntColorIndex As Byte = CInt("&H" & LstFrameHex(IntCursor + IntNumDrawingInstructions_colors_current))
                ...
                ' Rather than advancing the cursor one color at a time, do it at once now.
                ' IntNumDrawingInstructions_colors_current equals IntNumDrawingInstructions_colors here
                ' (VB's For loop leaves the counter one past the last value after a normal exit).
                If Me.IsShadowFormat = False Then
                    IntCursor += IntNumDrawingInstructions_colors_current
                End If
        ...
        ' Implemented a check for APE junk bytes left over at the end.
        ...
        If IntCursor < LstFrameHex.Count Then
        ...
        Catch ex As Exception
            ...
            Dim StrErrorMessage As String =
                ...
                "Remaining unprocessed bytes: " & (LstFrameHex.Count - IntCursor) & " (of " & LstFrameHex.Count & " total)" & vbCrLf &
